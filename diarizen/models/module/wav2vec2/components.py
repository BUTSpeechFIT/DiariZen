...
Modified in:
https://github.com/pyf98/DPHuBERT/blob/master/wav2vec2/components.py

Further modified by:
Jiangyu Han (ihan@fit.vut.cz), Brno University of Technology
...
class ConvLayerBlock(Module):
    ...
    def __init__(
        ...
        if prune_conv_channels:
            self.hard_concrete = HardConcrete(n_in=out_channels, init_mean=0.01, limit_l=limit_l, limit_r=limit_r, temperature=temperature)
        else:
    ...
    def get_num_macs_and_out_channels_length(self, in_channels, T_in):
        if self.hard_concrete is not None:
            out_channels = self.hard_concrete.l0_norm()
        else:
            out_channels = self.conv.out_channels
        ...
        T_out = int((T_in - (self.kernel_size - 1) - 1) / self.stride + 1)
        macs = T_out * self.kernel_size * in_channels * out_channels
        
        return macs, out_channels, T_out
...
class FeatureExtractor(Module):
    ...
    def get_num_macs_and_final_out_channels_length(self):
        in_channels = 1
        T_in = 16000    # 1s, 16khz
        num_macs = 0
        for layer in self.conv_layers:
            layer_macs, in_channels, T_in = layer.get_num_macs_and_out_channels_length(in_channels, T_in)
            num_macs += layer_macs
        
        return num_macs, in_channels, T_in
...
class FeatureProjection(Module):
    ...
    def get_num_macs(self, in_features, T_in):
        return T_in * in_features * self.projection.out_features
...
class ConvolutionalPositionalEmbedding(Module):
    ...
    def get_num_macs(self, T_in):
        # https://pytorch.org/docs/stable/generated/torch.nn.Conv1d.html#conv1d
        padding = self.conv.padding[0]
        groups = self.conv.groups
        T_out = int(T_in + 2 * padding - self.kernel_size + 1)
        macs = T_out * self.kernel_size * self.embed_dim * self.embed_dim // groups
...
class SelfAttention(Module):
    ...
    def __init__(
        ...
        if prune_heads:
            self.hard_concrete_for_heads = HardConcrete(n_in=num_heads, init_mean=0.01, limit_l=limit_l, limit_r=limit_r, temperature=temperature)
        else:
            self.hard_concrete_for_heads = None

        if prune_layer:
            self.hard_concrete_for_layer = HardConcrete(n_in=1, init_mean=0.01, limit_l=limit_l, limit_r=limit_r, temperature=temperature)
        else:
    ...
    def get_num_macs(self, T_in):
        if self.hard_concrete_for_heads is not None:
            ...
            num_heads = self.num_heads
        num_macs = 4 * T_in * num_heads * self.embed_dim * self.head_dim \
            + 2 * T_in ** 2 * num_heads * self.head_dim
            
        if self.hard_concrete_for_layer is not None:
            num_macs *= self.hard_concrete_for_layer.l0_norm()
...
class WavLMSelfAttention(SelfAttention):
    ...
    def __init__(
        ...
        super().__init__(embed_dim, len(self.remaining_heads), self.head_dim, dropout, prune_heads, prune_layer, limit_l=limit_l, limit_r=limit_r, temperature=temperature)
...
class FeedForward(Module):
    ...
    def get_num_macs(self, T_in):
        ...
        if self.hard_concrete_for_intermediate is not None:
            ...
            intermediate_features = self.intermediate_dense.out_features
        num_macs = 2 * T_in * io_features * intermediate_features
...
class EncoderLayer(Module):
    ...
    def get_num_macs(self, T_in):
        num_macs = 0
        if self.attention is not None:
            num_macs += self.attention.get_num_macs(T_in)
        if self.feed_forward is not None:
            num_macs += self.feed_forward.get_num_macs(T_in)
        return num_macs
...
class Transformer(Module):
    ...
    def get_num_macs(self, T_in):
        # pos_conv_embed and layer_norm
        num_macs = self.pos_conv_embed.get_num_macs(T_in)
        for layer in self.layers:
            num_macs += layer.get_num_macs(T_in)
        return num_macs
    ...
    def prune(self):
        ...
        for layer in self.layers:
            if layer.attention is not None:
                ...
                new_config["use_attention"].append(False)
            ...
            if layer.feed_forward is not None:
                ...
                new_config["use_feed_forward"].append(False)
...
class Encoder(Module):
    ...
    def get_num_macs(self, in_features, T_in):
        """Calculate the current model size."""
        feature_projection_macs = self.feature_projection.get_num_macs(in_features, T_in)
        transformer_macs = self.transformer.get_num_macs(T_in)
        return feature_projection_macs + transformer_macs
