...
Modified in:
https://github.com/pyf98/DPHuBERT/blob/master/wav2vec2/model.py
...
class Wav2Vec2Model(Module):
    ...
    def get_num_macs(self):
        """Calculate the current macs.""" 
        feature_extractor_macs, encoder_in_features, T_in = self.feature_extractor.get_num_macs_and_final_out_channels_length()
        encoder_macs = self.encoder.get_num_macs(encoder_in_features, T_in)
        return feature_extractor_macs + encoder_macs
...
def wavlm_model(
    ...
    feature_extractor = components._get_feature_extractor(
        extractor_mode, extractor_conv_layer_config, extractor_conv_bias,
        prune_conv_channels=extractor_prune_conv_channels,
        limit_l=limit_l, limit_r=limit_r, temperature=temperature
    )
