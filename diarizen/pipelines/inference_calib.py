...
class DiariZenPipeline(SpeakerDiarizationPipeline):
    ...
    def __call__(self, in_wav, sess_name=None):
        ...
        # Apply calibration to powerset probabilities
        ...
        # Convert logits to probabilities (softmax) - calibrator expects probabilities
        exp_logits = np.exp(logits_powerset - logits_powerset.max(axis=1, keepdims=True))  # numerical stability
        probs_powerset_input = exp_logits / exp_logits.sum(axis=1, keepdims=True)

        # Calibrate in powerset space
        probs_calibrated = self.calibrator(probs_powerset_input)

        # Binarize: argmax over powerset classes, then convert to multilabel
        mapping = self.powerset.mapping.cpu().numpy()  # (num_powerset_classes, num_classes)
        ...
        binarized_multilabel = hard_powerset @ mapping  # (N, num_classes)

        # Get soft multilabel for segmentations.data (used later in reconstruct)
        probs_multilabel = probs_calibrated @ mapping

        # Create segmentations object with multilabel shape
        num_classes = mapping.shape[1]
        multilabel_shape = (original_shape[0], original_shape[1], num_classes)
        ...
        # # Debug: check if any speakers detected
        # print(f"Binarized segmentations shape: {binarized_segmentations.data.shape}")
        # print(f"Active frames per speaker: {np.sum(binarized_segmentations.data, axis=(0,1))}")
        # print(f"Total active frames: {np.sum(binarized_segmentations.data)}")
