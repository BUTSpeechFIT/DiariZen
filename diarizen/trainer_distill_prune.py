...
class Trainer:
    def __init__(
        ...
        # Prune
        self.use_reg = self.trainer_config.get("use_reg", False)
        self.target_sparsity = self.trainer_config.get("target_sparsity", 0.60)
        self.pre_train_epochs = self.trainer_config.get("pre_train_epochs", 0)
        self.sparsity_warmup_epochs = self.trainer_config.get("sparsity_warmup_epochs", 5)

        self.target_sparsity_cur_epoch = self.trainer_config.get("target_sparsity_cur_epoch", 0)
        self.expected_sparsity_cur_epoch = self.trainer_config.get("expected_sparsity_cur_epoch", 0)

        # Distill
        self.further_distill = self.trainer_config.get("further_distill", False)
        self.apply_early_stop = False if not self.further_distill else True
    ...
    def train(self, train_dataloader: DataLoader, validation_dataloader):
        ...
        if self.warmup_steps > 0:
            self.create_schedulers(warmup_steps=self.warmup_steps, max_steps=max_steps)
        ...
        for epoch in range(self.state.epochs_trained + 1, max_epochs + 1):
            ...
            if self.state.epochs_trained > self.sparsity_warmup_epochs:
