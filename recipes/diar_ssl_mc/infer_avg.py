...
def att_enhanced_emb(att_score, embeddings):
    """
    att_score: [B, L, T, C, C]
    embeddings: [C, B, S, D]
    return: [B, S, D] — weighted speaker embedding per chunk
    """
    weighted_avg = np.mean(att_score[:, 3, :], (1,2))  # (B, C)
    embeddings = np.transpose(embeddings, (1, 2, 0, 3))     # [B, S, C, D]
    out = np.einsum('bj,bsjd->bsd', weighted_avg, embeddings)  # (B, S, D)

    print(f'att_score: {att_score.shape} | weighted_avg: {weighted_avg.shape} | out: {out.shape}')
    return out  # [B, S, D]
...
def diarize_session(
    ...
    segmentations, attention_score = pipeline.get_segmentations({"waveform": waveform, "sample_rate": sample_rate}, soft=False)
    ...
    for ch in range(waveform.shape[0]):
        print(f'CH={ch} | Extracting Embeddings...')
        waveform_ch = torch.unsqueeze(waveform[ch], 0) 
        embeddings_ch = pipeline.get_embeddings(
            {"waveform": waveform_ch, "sample_rate": sample_rate},
            binarized_segmentations,
            exclude_overlap=pipeline.embedding_exclude_overlap,
        )
        embeddings_list.append(embeddings_ch)

    embeddings = np.stack(embeddings_list, axis=0)
    embeddings = att_enhanced_emb(attention_score, embeddings)
